...
function share_to(website) {
    ...
    if (website === 'facebook') {
        ...
        var callback = function(url) {
            var link = document.createElement('a');
            link.href = "https://twitter.com/intent/tweet?ref_src=twsrc%5Etfw&text=Come%see%20my%20cool%20filtered%20image%20here:&tw_p=tweetbutton&url=" + encodeURIComponent(url);
...
/* Returns all of the user's galleries as a JSON object of the form:
   {
       "albums": [
           {
               "album_name": "---",
...
function get_galleries() {
    ...
    galleries = {
        'albums': [
            ...
            {
                'album_name': 'Baz',
                'images': [test, test, test, test, test, test, test]
            }
        ]
    };
    return galleries;
...
function setup_galleries() {
    if (is_homepage()) {   // Only home.html has the galleries
        var galleries = (get_galleries())['albums'];
